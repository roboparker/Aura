...
const uniqueEmail = () => `tags-${Date.now()}-${Math.random().toString(36).slice(2, 8)}@example.com`;
...
test.describe("Tags", () => {
  ...
  test("user can create, edit, and delete a tag", async ({ page }) => {
    ...
    await page.fill("#description", "High priority");
    await page.click('button[type="submit"]');
  ...
  test("nav shows Tags link when authenticated", async ({ page }) => {
    await registerAndSignIn(page, uniqueEmail());
