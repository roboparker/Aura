...
/**
 * Landing page for the account-export email link. The API resolves the
 * token only for the signed-in requester, so an unauthenticated click
 * routes through sign-in and comes back here; anyone else's session sees
 * "not found". The Download button is a plain navigation to the API's
 * streaming endpoint (same origin, session cookie included).
 */
const AccountExportDownloadPage = () => {
    ...
    void (async () => {
      try {
        const res = await fetch(
          `${ENTRYPOINT}/account-exports/${encodeURIComponent(exportToken)}`,
  ...
  if (notFound) {
    body = (
      <>
        ...
        <p className="text-sm text-muted-foreground">
          ...
          signed in as the person who requested the export — or you can request
    ...
    body = (
      <>
        ...
        <p className="text-sm text-muted-foreground">
          Data exports are kept for 7 days, and this one has been deleted. You
          can request a fresh export from your account settings.
        </p>
      </>
    );
    ...
    body = (
      <>
        ...
        <h1 className="text-xl font-semibold">Your data export is ready</h1>
        <p className="text-sm text-muted-foreground">
        ...
        <Button asChild className="gap-1.5">
          <a
            href={`${ENTRYPOINT}/account-exports/${encodeURIComponent(exportToken ?? "")}/download`}
          >
...
export default AccountExportDownloadPage;
