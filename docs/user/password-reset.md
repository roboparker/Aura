...
If you can't remember your Aura password, request a reset link by email. The link sets a new password — you don't need to know the old one.

## Reset your password

1. Open the sign-in page and click **Forgot password?**.
2. Enter the email address you use for Aura and submit.
3. Aura always says "if an account exists, we sent a link" — this same message shows whether or not we recognised the address, so nobody can use the form to fish for valid accounts.
4. Open your email and look for **"Reset your Aura password"** from `no-reply@aura.test` (or your deployment's configured sender).
...
Aura requires at least **8 characters** and rejects passwords that are too easy to guess. The strength meter under the new-password field shows how your choice scores. If it sits in the "too weak" range, the form won't submit.
...
If you click the email link and Aura tells you the link can't be used, it's one of four reasons:
...
Aura didn't recognise the link. Most commonly this means it was mangled by the email client (some clients break long URLs across lines), or you've requested another reset since this email landed (newer requests invalidate older ones). Request a fresh link.
...
- **Double-check the email address.** Aura's response is intentionally identical whether or not we recognised the email. If you mistyped or you actually signed up with a different address, you won't get an email.
...
**Can I change the email Aura sends reset links to?**
